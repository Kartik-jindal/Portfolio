    ...
    <section id="experience" ref={scrollRef} className="py-32 px-6 bg-white/[0.01]">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-16 md:gap-24">
        <div className="lg:w-1/3 sticky top-32 h-fit">
          <span className="text-primary uppercase tracking-[0.5em] text-[12px] font-black block mb-6">Evolution</span>
          <h2 className="text-7xl md:text-8xl font-headline font-black mb-8 tracking-tighter leading-none">Career <br /> <span className="text-outline italic">Path</span></h2>
          <p className="text-muted-foreground text-xl md:text-2xl font-body font-light leading-relaxed">
        ...
        <div className="lg:w-2/3 relative">
          ...
          <div className="space-y-24">
            {experiences.map((exp, i) => (
              ...
              >
                <div className="absolute left-0 top-6 -translate-x-1/2 w-5 h-5 bg-primary shadow-[0_0_25px_rgba(16,185,129,0.6)]" />
                
                <div className="flex flex-col md:flex-row md:items-baseline justify-between gap-6 mb-8">
                  <div>
                    <h3 className="text-4xl md:text-6xl font-headline font-bold text-white mb-3">{exp.company}</h3>
                    <span className="text-primary font-black text-sm md:text-base tracking-[0.4em] uppercase">{exp.role}</span>
                  </div>
                  <div className="text-muted-foreground text-sm font-black tracking-widest uppercase border border-white/10 px-8 py-3 w-fit">
                    {exp.period}
                  </div>
                </div>
                <p className="text-muted-foreground text-xl md:text-2xl leading-relaxed max-w-3xl font-light font-body">
                  {exp.desc}
                </p>
              </motion.div>
            ))}
