...
import { ArrowUpRight, Github, Code2, Globe, Cpu, X, ExternalLink, Box, Terminal, Activity, Zap, Shield, Database, Sparkles, Binary } from 'lucide-react';
  ...
  return (
    <main className="bg-transparent min-h-screen">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {smallProjects.map((project, i) => (
              <motion.div
                ...
                className="glass p-8 rounded-3xl border-white/5 hover:border-primary/30 transition-all duration-500 group relative flex flex-col justify-between h-[440px] cursor-pointer"
              >
                ...
                <div className="space-y-4 pt-4">
                  ...
                  <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-white/40 group-hover:text-primary transition-colors">
                    View Lab Report <ArrowUpRight className="w-3 h-3" />
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <Dialog open={!!selectedProject} onOpenChange={() => setSelectedProject(null)}>
        <DialogContent className="max-w-5xl bg-background/98 backdrop-blur-3xl border-white/5 p-0 overflow-hidden rounded-[2.5rem] shadow-2xl outline-none">
          <AnimatePresence>
            {selectedProject && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="h-full">
                {/* Immersive Header */}
                <div className="relative h-80 w-full overflow-hidden">
                  <Image src={selectedProject.image} alt={selectedProject.title} fill className="object-cover opacity-30 grayscale group-hover:grayscale-0 transition-all" />
                  ...
                  <div className="absolute bottom-10 left-12 right-12 z-20">
                     <div className="flex items-center gap-4 mb-3">
                        <span className="text-primary font-black tracking-[0.4em] text-[10px] uppercase bg-primary/10 px-3 py-1 rounded-md border border-primary/20">Lab_Report_{selectedProject.type}</span>
                        <div className="h-px flex-1 bg-white/10" />
                     </div>
                     <DialogTitle className="text-5xl md:text-7xl font-headline font-black text-white italic tracking-tighter">{selectedProject.title}</DialogTitle>
                  </div>
                </div>
                    ...
                    <div className="lg:col-span-8 space-y-12">
                      <div className="space-y-8">
                        <div className="flex items-center gap-3 text-primary">
                          ...
                          <h4 className="text-[10px] font-black uppercase tracking-[0.5em]">Technical Narrative</h4>
                        </div>
                      ...
                      <div className="p-8 rounded-3xl bg-white/[0.02] border border-white/5 space-y-6">
                        <div className="flex items-center gap-3 text-accent/50">
                          ...
                          <h4 className="text-[10px] font-black uppercase tracking-[0.5em]">Architectural Insights</h4>
                        </div>
                        <p className="text-sm text-white/50 leading-relaxed italic">
                          This experiment emphasizes the balance between computational efficiency and aesthetic fluidity. Every line of code was architected to ensure that visual complexity does not compromise the underlying system stability, a core pillar of high-fidelity digital engineering.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="lg:col-span-4 space-y-12">
                      <div className="space-y-8 p-10 rounded-[2rem] glass-accent border-primary/10">
                        <div className="space-y-4">
                          <h4 className="text-[10px] font-black uppercase tracking-[0.5em] text-white/30">Architecture Stack</h4>
                          <div className="flex flex-wrap gap-2">
                        ...
                        <div className="space-y-4">
                          <h4 className="text-[10px] font-black uppercase tracking-[0.5em] text-white/30">Release Phase</h4>
                          <div className="flex items-center gap-3">
                            <div className="w-3 h-3 rounded-full bg-primary animate-pulse" />
                            <span className="text-sm font-bold text-white">Active Research</span>
                          </div>
                        </div>
                        ...
                        >
                          Access Repo <ArrowUpRight className="w-4 h-4 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
                        </a>
                      </div>
                      
                      <div className="flex items-center justify-between px-6">
                        <span className="text-[9px] font-black text-white/20 tracking-[0.4em] uppercase">Document_042</span>
                        <div className="flex gap-4">
                           <Github className="w-4 h-4 text-white/20 hover:text-white transition-colors cursor-pointer" />
                           <Globe className="w-4 h-4 text-white/20 hover:text-white transition-colors cursor-pointer" />
                        </div>
                      </div>
                    </div>
                  </div>
