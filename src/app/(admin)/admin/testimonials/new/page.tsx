...
export default function NewTestimonialPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: 'Voice Captured', description: 'Social proof entry initialized.' });
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <h1 className="text-5xl font-headline font-black italic tracking-tighter text-white">Capture Voice.</h1>
          </div>
        </div>
        <Button 
        ...
        >
          {loading ? 'Initializing...' : 'Store Voice'} <Save className="w-5 h-5 ml-2 group-hover:scale-110 transition-transform" />
        </Button>
      </header>
        ...
        <div className="glass p-10 rounded-[2.5rem] border-white/5 space-y-8">
          ...
          <div className="space-y-2">
            ...
            <Textarea 
              ...
              placeholder="The project exceeded all architectural expectations..." 
            />
          </div>
        </div>
      </div>
